...
public class MagnetSelectorTests
{
    ...
    // ------------------------------------------------------------------
    // availability-signal-prowlarr-fallback: ProbeAvailabilityWithFallbackAsync.
    // Root cause (proven live): Torrentio returns HTTP 429 for any id it
    // cannot serve (mainstream=200; obscure/anime/unknown=429; NOT a rate
    // limit — 15 rapid good-id requests all 200). The plugin's availability
    // sweep must not sink a title Prowlarr actually has into a permanent
    // IndeterminateTransient/24h-backoff loop.
    // ------------------------------------------------------------------

    private static PluginConfiguration FallbackTestConfig() => new()
    {
        MinSeeders = 1,
        MinSizeGb1080p = 1,
        MinSizeGb4K = 1,
        AvailabilityOracleFailureRetries = 1,
        AvailabilityOracleRetryDelayMilliseconds = 0,
    };

    [Fact]
    public async Task ProbeAvailabilityWithFallback_TorrentioServesTitle_NeverInvokesProwlarr()
    {
        // Mainstream/Torrentio-served title: cadence gate — the heavy Prowlarr
        // fallback path must NEVER run when the oracle itself succeeds.
        var torrentio = new Mock<IIndexerClient>(MockBehavior.Strict);
        torrentio.SetupGet(i => i.IsEnabled).Returns(true);
        torrentio.SetupGet(i => i.Name).Returns("Torrentio");
        torrentio.SetupGet(i => i.IsAvailabilityOracle).Returns(true);
        ...
        prowlarr.SetupGet(i => i.Name).Returns("Prowlarr");
        prowlarr.SetupGet(i => i.IsAvailabilityOracle).Returns(false);
        // No SearchAsync setup: any call fails the (Strict) mock — proves the
        // fallback path never engages for a title Torrentio actually serves.

        var scorer = new Materialisation.QualityScorer(NullLogger<Materialisation.QualityScorer>.Instance);
        var cfg = FallbackTestConfig();
        var sel = new MagnetSelector(new[] { torrentio.Object, prowlarr.Object }, scorer, NullLogger<MagnetSelector>.Instance, () => cfg);

        var probe = await sel.ProbeAvailabilityWithFallbackAsync(1, "tt1234567", "movie", null, null, "Movie", 2020, CancellationToken.None);
    ...
    [Fact]
    public async Task ProbeAvailabilityWithFallback_TorrentioHttpFailurePersists_FallsBackToProwlarrAndBecomesAvailable()
    {
        // Reproduces the exact live root cause: Torrentio returns HTTP 429 for
        // an id it cannot serve (obscure/anime/series-as-movie/unknown), on
        // every attempt including the bounded retry (proven NOT a rate limit
        // live: 15 rapid good-id requests all returned 200). Prowlarr HAS the
        // content — the fallback must confirm it Available.
        var torrentio = new Mock<IIndexerClient>(MockBehavior.Strict);
        torrentio.SetupGet(i => i.IsEnabled).Returns(true);
        torrentio.SetupGet(i => i.Name).Returns("Torrentio");
        torrentio.SetupGet(i => i.IsAvailabilityOracle).Returns(true);
        torrentio.Setup(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new IndexerTransientException("Torrentio returned HTTP 429"));
        ...
        prowlarr.SetupGet(i => i.IsAvailabilityOracle).Returns(false);
        prowlarr.Setup(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { MakeCandidate("Adventure Time Fionna and Cake S01 1080p", 5, 338) });

        var scorer = new Materialisation.QualityScorer(NullLogger<Materialisation.QualityScorer>.Instance);
        var cfg = FallbackTestConfig();
        var sel = new MagnetSelector(new[] { torrentio.Object, prowlarr.Object }, scorer, NullLogger<MagnetSelector>.Instance, () => cfg);

        var probe = await sel.ProbeAvailabilityWithFallbackAsync(1, "tt9999998", "movie", null, null, "Adventure Time: Fionna and Cake", 2023, CancellationToken.None);

        Assert.Equal(MagnetProbeOutcome.Available, probe.Outcome);
        Assert.Single(probe.Candidates);
        Assert.Equal(338, probe.Candidates[0].Seeders);
        // Torrentio: initial attempt + AvailabilityOracleFailureRetries (1) bounded retries = 2 calls total.
        torrentio.Verify(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()), Times.Exactly(2));
        prowlarr.Verify(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task ProbeAvailabilityWithFallback_TorrentioHttpFailureAndProwlarrAlsoAbsent_StaysTransient()
    {
        // Neither indexer has the content: fallback must not invent
        // availability — the ORIGINAL oracle transient outcome is preserved
        ...
        var probe = await sel.ProbeAvailabilityWithFallbackAsync(1, "tt0000000", "movie", null, null, "Nothing Serves This", 2023, CancellationToken.None);

        // Both indexers found nothing: falls through to DefinitiveUnavailable
        // via the fallback path (Prowlarr ran and returned zero candidates —
        // a real, definitive "no one has it" signal, not a transient error).
        Assert.Equal(MagnetProbeOutcome.DefinitiveUnavailable, probe.Outcome);
        prowlarr.Verify(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task ProbeAvailabilityWithFallback_ProwlarrAlsoFailsTransiently_PreservesOriginalOracleTransient()
    {
        // Both the oracle AND the fallback are inconclusive (transient
        // failures) — must return the ORIGINAL oracle outcome so the normal
        // transient-backoff cadence still governs (never invents Available or
        // DefinitiveUnavailable from two failures).
        var torrentio = new Mock<IIndexerClient>(MockBehavior.Strict);
        torrentio.SetupGet(i => i.IsEnabled).Returns(true);
        torrentio.SetupGet(i => i.Name).Returns("Torrentio");
        torrentio.SetupGet(i => i.IsAvailabilityOracle).Returns(true);
        ...
        prowlarr.SetupGet(i => i.IsAvailabilityOracle).Returns(false);
        prowlarr.Setup(i => i.SearchAsync(It.IsAny<IndexerQuery>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new IndexerTransientException("Prowlarr transport failure"));

        var scorer = new Materialisation.QualityScorer(NullLogger<Materialisation.QualityScorer>.Instance);
        var cfg = FallbackTestConfig();
        var sel = new MagnetSelector(new[] { torrentio.Object, prowlarr.Object }, scorer, NullLogger<MagnetSelector>.Instance, () => cfg);

        var probe = await sel.ProbeAvailabilityWithFallbackAsync(1, "tt0000001", "movie", null, null, "Flaky Everywhere", 2023, CancellationToken.None);

        Assert.Equal(MagnetProbeOutcome.IndeterminateTransient, probe.Outcome);
        Assert.Equal("indexer_partial_or_total_failure", probe.ErrorKind);
    }

    [Fact]
    public async Task ProbeAvailabilityWithFallback_NoCapableIndexer_NeverRetriesOrFallsBack()
    {
        // NoCapableIndexer (no imdb id / all abstained) is untouched by this
        // fallback — no bounded retry, no Prowlarr fallback: same behaviour
        // as plain ProbeAvailabilityAsync.
        var torrentio = new Mock<IIndexerClient>(MockBehavior.Strict);
        torrentio.SetupGet(i => i.IsEnabled).Returns(true);
        torrentio.SetupGet(i => i.Name).Returns("Torrentio");
        torrentio.SetupGet(i => i.IsAvailabilityOracle).Returns(true);
        ...
        var probe = await sel.ProbeAvailabilityWithFallbackAsync(1, null, "movie", null, null, "Movie", 2020, CancellationToken.None);
