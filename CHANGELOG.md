...
- Added Vault Mode (REQ-M14-VAULT): favouriting a Phantom movie/episode now materialises then asks gostream to prestage the stub so it stays resident on the gostream box; de-favouriting or evicting it releases that footprint via gostream unprestage. Gated on the discrete favourite/rating save reasons (not playback ticks) and on a new `Vault Mode` admin setting (`VaultModeEnabled`, default on) plus a `VaultPrestagePriority` (default 50). Prestage stops when Vault Mode is disabled; unprestage keeps running so a disabled Vault Mode drains rather than strands prestaged bytes. Movie and TV episode parity. Requires a gostream build exposing Vault Mode endpoints; when absent, every Vault call is a no-op.
